...
func Routes(router *gin.Engine) { // Import the missing package

	app := app.NewApplication(database.ProductData(database.Client, "Products"), database.UserData(database.Client, "Users"), database.OrderData(database.Client, "Orders"), database.PayloadData(database.Client, "Payloads"), database.MessageData(database.Client, "Messages"))
